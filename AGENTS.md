...
- Restart and verify in this order: `launchctl bootout gui/$(id -u)/com.zcode-as-subagent.daemon`, `launchctl bootstrap gui/$(id -u) /Users/ibobby/Library/LaunchAgents/com.zcode-as-subagent.daemon.plist`, then `launchctl print gui/$(id -u)/com.zcode-as-subagent.daemon`, check the daemon socket, and run `node bin/zas.mjs status` or a scoped `list` call.
