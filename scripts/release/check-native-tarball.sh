...
file npm/native/darwin-arm64/zcode-as-subagentd | grep -q 'Mach-O 64-bit executable arm64'
test "$(stat -f '%Lp' npm/native/darwin-arm64/zcode-as-subagentd)" = 755
shasum -a 256 npm/native/darwin-arm64/zcode-as-subagentd "$package_file" > .agent-work/evidence/npm-tarball/sha256sums.txt
