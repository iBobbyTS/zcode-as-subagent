...
try {
  ...
  console.log(JSON.stringify({ ok: false, code: 'AGENT_FILE_POLICY_UNVERIFIED' }));
  process.exit(1);
}
const expectedScripts = {
  PreToolUse: [['^(Read|Grep|Glob|Write|Edit|Delete|Move)$', path.join(hookRoot, 'hooks', 'check-agent-files.mjs')]],
...
console.log(JSON.stringify({ ok: Boolean(ok), code: ok ? 'AGENT_FILE_POLICY_VERIFIED' : 'AGENT_FILE_POLICY_UNVERIFIED', provenance }));
process.exit(ok ? 0 : 1);
